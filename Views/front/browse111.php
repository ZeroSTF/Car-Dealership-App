...
if ( null==$code_reservation ) { header("Location: browse11.php"); }
...
{ 
           ...
           { $code_reservationErr = 'Please enter code_reservation'; $valid = false; }
          
          else if (!preg_match("/^[0-9]*$/",$code_reservation)) 
          {  $code_reservationErr = "Only Number space allowed"; } 
          ...
          if(empty($heure_res))
          {  $heure_resErr='Please Enter Heures de reservations '; $valid= false; }
          if(empty( $nb_jours))
          {  $nb_joursErr='Please Enter Nombre de jours '; $valid= false; }
          if(empty( $prix_res))
          {  $prix_resErr='Please Enter Prix '; $valid= false; }
          if ($valid) {
            $pdo = DatabaseConnect::connect(); 
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
             $sql = "UPDATE voitures_a_louer SET date_reservation = ?,nb_jours = ?, heure_res = ?, prix_res = ?  WHERE code_reservation=".$_GET['id'];
             $q = $pdo->prepare($sql);
...
<br />
<form method="post" action="edit.php?id=<?php echo $code_reservation ;?>">
