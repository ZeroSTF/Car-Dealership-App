...
if ( null==$id ) { header("Location: browse10.php"); }
...
{ 
           ...
           $bin = htmlentities(trim($_POST['bin']));
        
           if (empty($id)) 
           { $idErr = 'Please enter id'; $valid = false; }
          
          else if (!preg_match("/^[0-9]*$/",$id)) 
          {  $idErr = "Only Number space allowed"; } 
          
          
          if(empty($nom))
          {  $nomErr='Please Enter name '; $valid= false; }
          
          if(empty($prix))
          {  $prixErr='Please Enter price '; $valid= false; }
          if(empty( $description))
          {  $descriptionErr='Please Enter description '; $valid= false; }
          if(empty( $bin))
          {  $binErr='Please Enter picture '; $valid= false; }
          if ($valid) {
            $pdo = DatabaseConnect::connect(); 
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
             $sql = "UPDATE voitures_a_louer SET nom = ?,description = ?, prix = ?, bin = ?  WHERE id=".$_GET['id'];
             $q = $pdo->prepare($sql);
             $q->execute(array( $nom, $description, $prix, $bin));
...
<br />
<form method="post" action="browse13.php?id=<?php echo $id ;?>">
...
<br />
<div class="control-group <?php echo !empty($prixErr)?'error':'';?>">
                        <label class="control-label">Price</label>

<br />
<div class="controls">
                            <input name="prix" type="number" placeholder="Email Address" value="<?php echo !empty($prix)?$prix:'';?>">
...
<br />
<div class="controls">
                            <input name="bin" type="file" value="<?php echo !empty($bin) ? $bin:'';?>">
                            <?php if (!empty($binErr)): ?>
...
<div class="form-actions">
                 ...
                 <a class="btn" href="browse10.php">Get back</a>
</div>
<p>
